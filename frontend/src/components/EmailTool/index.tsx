...
const PublicEmailTool: React.FC = () => {
  ...
  return (
    ...
    >
      <h1 className="text-foreground mb-6 text-center font-bold">
      ...
      {!cookiesEnabled && (
        <div className="bg-destructive-foreground text-destructive mb-4 rounded-md p-2">
      ...
      <label
        htmlFor="endpoint"
        className="text-foreground mb-2 mt-4 block text-lg font-semibold"
      >
      ...
      <select
        ...
        className="border-border bg-background text-foreground dark:bg-muted dark:text-muted-foreground w-full rounded-md border px-4 py-2 text-sm shadow-sm focus:border-primary focus:ring-primary"
      ...
      <div className="flex flex-wrap gap-2">
        {issues.split(",").map((issue, index) => (
          <div
            key={index}
            className="text-primary-foreground inline-flex items-center rounded-md bg-primary px-2 py-1 text-sm"
          >
            {issue.trim()}
          </div>
        ))}
      ...
      <input
        ...
        className="border-border bg-background text-foreground dark:bg-muted dark:text-muted-foreground mt-4 w-full rounded-md border px-4 py-2 text-sm shadow-sm focus:border-primary focus:ring-primary"
        disabled={!cookiesEnabled}
      />
      {errors.issues && (
        <p className="text-destructive text-sm">{errors.issues}</p>
      )}
      ...
      />
      {errors.fromName && (
        <p className="text-destructive text-sm">{errors.fromName}</p>
      )}
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          {errors.province && (
            <p className="text-destructive text-sm">{errors.province}</p>
          )}
        </div>
        <div>
          <label
      ...
      {errors.followup && (
        <p className="text-destructive text-sm">{errors.followup}</p>
      )}
      ...
      {searchQuery && filteredContacts.length > 0 && (
        <ul className="border-border bg-background dark:bg-muted mt-4 max-h-60 overflow-y-auto rounded-md border p-2 text-xs shadow-sm sm:max-h-40 sm:text-sm">
          {filteredContacts.map((contact, index) => (
            <li
              key={index}
              className="hover:bg-muted dark:hover:bg-muted-foreground cursor-pointer p-2"
              onClick={() => {
                setSearchQuery(
      ...
      <input
        ...
        readOnly
        autoComplete="off"
        className="border-border bg-background text-foreground dark:bg-muted dark:text-muted-foreground mt-4 w-full cursor-not-allowed rounded-md border px-4 py-2 text-sm shadow-sm focus:border-primary focus:ring-primary"
        placeholder="MP/MPP/MLA's Email will appear here"
      />
      {errors.toEmail && (
        <p className="text-destructive text-sm">{errors.toEmail}</p>
      )}
      ...
      {showSuccessAlert && (
        <div className="bg-accent text-accent-foreground mt-4 rounded-md p-2">
          Data copied to clipboard successfully!
        </div>
      )}
      <div
        className="mt-6 max-w-full overflow-auto break-words"
        id="StreamingContent"
        ref={dataRef}
